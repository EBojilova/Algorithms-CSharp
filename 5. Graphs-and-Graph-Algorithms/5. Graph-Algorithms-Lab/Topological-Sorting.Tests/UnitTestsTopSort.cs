...
namespace Topological_Sorting_DFS.Tests
{
    [TestClass]
    public class UnitTestsTopSort
    {
        ...
        {
            // Arrange
            var graph = new Dictionary<string, List<string>> { { "A", new List<string> {} } };
        ...
        public void TestPerformanceGraph1000Vertices()
        {
            ...
            for (var i = 0; i < nodesCount - 50; i++)
            {
                for (var c = 25; c < i % 50; c++)
                {
        ...
        private void AssertTopologicallySorted(Dictionary<string, List<string>> graph, List<string> sortedNodes)
        {
            foreach (var node in graph)
            {
                foreach (var childNode in node.Value)
                {
                    var nodeIndex = sortedNodes.IndexOf(node.Key);
                    var childIndex = sortedNodes.IndexOf(childNode);
                    Assert.IsTrue(nodeIndex != -1, "Node " + node.Key + " not found.");
                    Assert.IsTrue(childIndex != -1, "Node " + childNode + " not found.");
                    Assert.IsTrue(nodeIndex < childIndex, "Node " + node.Key + " should come before " + childNode);
                }
            }
        }
    }
