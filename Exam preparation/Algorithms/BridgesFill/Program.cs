namespace BridgesFill
{
    ...
    public class Bridges
    {
        ...
        {
            var northSide = Console.ReadLine()
                .Split()
                .Select(int.Parse)
                .ToArray();

            var southSide = Console.ReadLine()
                .Split()
                .Select(int.Parse)
                .ToArray();

            var maxBridges = new int[northSide.Length, southSide.Length];

            maxBridges[0, 0] = northSide[0] == southSide[0] ? 1 : 0;

            // Initialize first row
            for (var col = 1; col < maxBridges.GetLength(1); col++)
            {
            ...
            for (var row = 1; row < maxBridges.GetLength(0); row++)
            {
